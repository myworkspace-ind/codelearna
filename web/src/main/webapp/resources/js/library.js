		...
		courseItems.forEach(function(courseItem) {
			...
			fetch(`${_ctx}library/course/progress?courseId=${courseId}`)
				...
				.then(data => {
					...
					let cardBody = courseItem.querySelector('.card-body');
					if (!cardBody) {
						const closestCard = courseItem.closest('.card');
						cardBody = closestCard ? closestCard.querySelector('.card-body') : courseItem;
					}
					...
					progressDiv.innerHTML = 
					    '<div class="progress mb-2" style="height: 20px;">' +
					        ...
					        ' role="progressbar" style="width: ' + data.completionPercentage + '%"' +
